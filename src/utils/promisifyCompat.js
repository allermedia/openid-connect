 ...
 */
export default function safePromisify(method, context) {
  ...
  const methodSource = method.toString();
  const directPromisePatterns = ['return Promise.resolve', 'return Promise.reject', 'return new Promise'];

  const returnsDirectPromise = directPromisePatterns.some((pattern) => methodSource.includes(pattern));
