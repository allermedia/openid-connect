...
function requiresLoginMiddleware(requiresLoginCheck, req, res, next) {
  ...
  if (requiresLoginCheck(req)) {
    if (!res.oidc.errorOnRequiredAuth && req.accepts('html')) {
      debugRequiresAuth('authentication requirements not met with errorOnRequiredAuth() returning false, calling res.oidc.login()');
...
export function claimIncludes(claim, ...expected) {
  ...
  const authenticationCheck = (req) => {
    ...
    if (typeof actual === 'string') {
      ...
      debugRequiresAuth('unexpected claim type. expected array or string, got %o', typeof actual);
      return true;
    }
