...
import { regenerateSessionStoreId, replaceSession } from '../src/appSession.js';
...
import { cancelSilentLogin, resumeSilentLogin } from './middleware/attemptSilentLogin.js';
...
async function refresh({ tokenEndpointParams } = {}) {
  ...
  if (newTokenSet.expires_in && typeof newTokenSet.expires_in === 'number') {
    sessionUpdate.expires_at = Math.floor(Date.now() / 1000) + newTokenSet.expires_in;
  }
...
export class RequestContext {
  ...
  get accessToken() {
    try {
      ...
      const expires_in = session.expires_at ? Math.max(0, session.expires_at - Math.floor(Date.now() / 1000)) : undefined;
  ...
  get idTokenClaims() {
    try {
      ...
      return { ...JSON.parse(JSON.stringify(claims)), sid: session.sid || claims.sid };
    } catch {
      return undefined;
    }
  }
  ...
  get user() {
    try {
      ...
      const user = JSON.parse(JSON.stringify(idTokenClaims));
...
export class ResponseContext {
  ...
  async login(options = {}) {
    ...
    try {
      ...
      if (usePKCE) {
        debugContext('response_type includes code, the authorization request will use PKCE');
        authVerification.code_verifier = transient.generateCodeVerifier();

        authParams.code_challenge_method = 'S256';
        authParams.code_challenge = await transient.calculateCodeChallenge(authVerification.code_verifier);
      }

      if (config.pushedAuthorizationRequests) {
        const { request_uri } = await client.pushedAuthorizationRequest(authParams, {
  ...
  async logout(params = {}) {
    ...
    try {
      ...
      const getLogoutUrl = (idTokenHint) => {
        ...
        if (!config.idpLogout) {
          debugContext('performing a local only logout, redirecting to %s', returnURL);
          return returnURL;
        }
      ...
      if (!req.oidc.isAuthenticated()) {
        debugContext('end-user already logged out, redirecting to %s', returnURL);
    ...
    debugContext('logging out of identity provider, redirecting to %s', returnURL);
  ...
  async callback(options = {}) {
    ...
    try {
      ...
      const session = Object.assign({}, tokenSet);
      ...
      const existingUser = existingSession && existingSession.id_token ? jose.decodeJwt(existingSession.id_token).sub : null;
      ...
      if (config.afterCallback) {
        const updatedSession = await config.afterCallback(req, res, req[config.session.name], req.openidState);
      ...
      const redirectTo = (req.openidState && req.openidState.returnTo) || config.baseURL;
  ...
  async backchannelLogout() {
    const { config, req, res } = weakRef(this);
    ...
    if (config.backchannelLogout && config.backchannelLogout.isInsecure) {
      ...
      debugContext('Using insecure backchannel logout mode - DO NOT USE IN PRODUCTION');
    ...
    try {
      ...
      try {
        ...
        const { payload, protectedHeader: header } = await jose.jwtVerify(logoutToken, jwks, {
        ...
        if (!verifiedToken.events || !verifiedToken.events['http://schemas.openid.net/event/backchannel-logout']) {
        ...
        debugContext('Logout token verification failed: %s', verificationError.message);
        res.status(400).json({
          error: 'invalid_token',
