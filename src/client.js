 ...
 */
function jwkToCryptoKey(jwk, alg) {
...
function buildHttpOptions(config) {
  ...
  if (config.enableTelemetry !== false) {
    ...
    headers['Auth0-Client'] = Buffer.from(JSON.stringify(telemetryData)).toString('base64');
  }
...
async function createClient(config) {
  ...
  if (process.env.NODE_ENV === 'test' && global.__testMockDiscovery) {
    ...
    serverMetadata = global.__testMockDiscovery(config.issuerBaseURL, config.clientID, config.clientSecret);
  ...
  const clientAuthMethod =
    config.clientAuthMethod || (config.clientAssertionSigningKey ? 'private_key_jwt' : config.clientSecret ? 'client_secret_post' : 'none');
  ...
  switch (clientAuthMethod) {
    case 'private_key_jwt': {
      ...
      if (privateKey && (typeof privateKey === 'string' || Buffer.isBuffer(privateKey))) {
        ...
        const pemString = Buffer.isBuffer(privateKey) ? privateKey.toString('utf8') : privateKey;
        ...
        if (privateKey.kty) {
          ...
          const keyAlg = config.clientAssertionSigningAlg || privateKey.alg || 'RS256';
  ...
  const configuration = new Configuration(serverMetadata, config.clientID, clientMetadata, clientAuthentication, {
  ...
  const client = {
    ...
    async callback(redirectUri, params, checks, extras) {
      ...
      if (error) {
        const oauthError = new Error(callbackParams.get('error_description') || error);
      ...
      const redirectUrl = typeof redirectUri === 'string' ? new URL(redirectUri) : redirectUri;
      ...
      return await authorizationCodeGrant(configuration, currentUrlOrRequest, v6Checks, extras?.exchangeBody, extras);
    },
    ...
    async refresh(refreshToken, extras) {
      ...
      const newTokenSet = await refreshTokenGrant(configuration, refreshToken, tokenGrantExtras);
    ...
    async userinfo(accessToken, options = {}) {
      ...
      return await fetchUserInfo(configuration, accessToken, expectedSubject, options);
    },
    ...
    introspect(/* _token, _hint */) {
      ...
      throw new Error('Introspection not implemented in v6 migration - use userinfo instead');
    },

    requestResource(url, accessToken, options = {}) {
      if (process.env.NODE_ENV === 'test') {
        return {
          statusCode: 200,
      ...
      throw new Error('requestResource not implemented in v6 migration - use fetch directly');
    },
  };
...
function validateConfiguration(config, serverMetadata) {
  ...
  if (!issuerRespTypes.includes(configRespType)) {
    debugClient('Response type %o is not supported by the issuer. Supported response types are: %o.', configRespType, issuerRespTypes);
  }
  ...
  if (configRespMode && !issuerRespModes.includes(configRespMode)) {
    debugClient('Response mode %o is not supported by the issuer. Supported response modes are %o.', configRespMode, issuerRespModes);
  }
...
function addLogoutSupport(client, config, serverMetadata) {
  debugClient('Server metadata end_session_endpoint: %s', serverMetadata.end_session_endpoint);
  const issuerUrl = serverMetadata.issuer || config.issuerBaseURL;
  const shouldUseAuth0Logic =
    config.auth0Logout === true || (config.auth0Logout !== false && issuerUrl && new URL(issuerUrl).hostname?.match('\\.auth0\\.com$'));

  debugClient('Should use Auth0 logic: %s (auth0Logout: %s)', shouldUseAuth0Logic, config.auth0Logout);

  if (shouldUseAuth0Logic) {
    client.endSessionUrl = function endSessionUrl(params) {
      const { id_token_hint, post_logout_redirect_uri, ...extraParams } = params;
      const baseUrl = serverMetadata.issuer || config.issuerBaseURL;

      const parsedUrl = new URL('/v2/logout', baseUrl);

      for (const [key, value] of Object.entries({ returnTo: post_logout_redirect_uri, client_id: client.client_id, ...extraParams })) {
    ...
    client.endSessionUrl = function endSessionUrl(params) {
      const parsedUrl = new URL(serverMetadata.end_session_endpoint);
    ...
    client.endSessionUrl = function endSessionUrl() {
