 ...
 */
export const createWellKnownMock = (issuerBaseUrl = DEFAULT_ISSUER_BASE_URL, overrides = {}) => {
  const config = {
    ...wellKnown,
  ...
  return nock(issuerBaseUrl).persist().get('/.well-known/openid-configuration').reply(200, config);
};

/**
 * Create a JWKS mock that works with different OIDC versions
 */
export function createJWKSMock(issuerBaseUrl = DEFAULT_ISSUER_BASE_URL, jwks = certs.jwks) {
  return nock(issuerBaseUrl).persist().get('/.well-known/jwks.json').reply(200, jwks);
}

/**
 * Create a token endpoint mock with configurable responses
 */
export function createTokenEndpointMock(issuerBaseUrl = DEFAULT_ISSUER_BASE_URL, tokenResponse = {}, options = {}) {
 ...
 */
export const createUserInfoMock = (issuerBaseUrl = DEFAULT_ISSUER_BASE_URL, userInfo = {}) => {
  ...
  return nock(issuerBaseUrl).persist().get('/userinfo').reply(200, defaultUserInfo);
};

/**
 * Create introspection endpoint mock
 */
export const createIntrospectionMock = (issuerBaseUrl = DEFAULT_ISSUER_BASE_URL) => {
...
export function createMockClient(options = {}) {
  ...
  const mockClient = {
    ...
    requestResource(/* url, token, */ options = {}) {
 ...
 */
export function setupOIDCMocks(options = {}) {
  const {
    issuerBaseUrl = DEFAULT_ISSUER_BASE_URL,
    wellKnownOverrides = {},
    jwks = certs.jwks,
  ...
  if (includeTokenEndpoint) {
    mocks.token = createTokenEndpointMock(issuerBaseUrl, tokenResponse, tokenOptions);
  }
 ...
 */
export function cleanupOIDCMocks() {
 ...
 */
export const createCallbackTokenMock = (issuerBaseUrl = DEFAULT_ISSUER_BASE_URL, responseHandler = null) => {
