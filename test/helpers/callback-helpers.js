 ...
 */
export function setupCallbackTest(params = {}) {
  const { authOpts = {}, body = {}, tokenResponseHandler, customTokenResponse = {} } = params;
  ...
  function defaultTokenHandler(_uri, requestBody) {
    tokenReqHeader = this.req.headers;
    tokenReqBody = requestBody;
    // tokenReqBodyJson = qs.parse(requestBody);
    tokenReqBodyJson = new URLSearchParams(requestBody);
  ...
  return {
    ...
    getTokenRequest() {
      return {
        ...
        body: tokenReqBody,
        bodyJson: Object.fromEntries(tokenReqBodyJson),
      };
    },
  };
}
 ...
 */
export function createCallbackCookies(values, customTxnCookieName = 'auth_verification') {
  return {
 ...
 */
export function expectCallbackSuccess(result, expectations = {}) {
  const {
    hasUser = true,
    hasTokens = true,
  ...
  if (result.response.statusCode !== redirectStatus) {
    throw new Error(`Expected status ${redirectStatus}, got ${result.response.statusCode}`);
  }
  ...
  if (result.currentUser) {
    for (const [claim, expectedValue] of Object.entries(userClaims)) {
      if (result.currentUser[claim] !== expectedValue) {
        throw new Error(`Expected user.${claim} to be ${expectedValue}, got ${result.currentUser[claim]}`);
      }
    }
  }
 ...
 */
export function mockCallbackScenario(scenario = 'success', customOptions = {}) {
  const scenarios = {
    success: {
 ...
 */
export function validateCallbackResult(result, expectedOutcome) {
