 ...
 */
function verifyCookie(cookie, value, keystore) {
  if (!value) return;
 ...
 */
function signCookie(cookie, value, key) {
 ...
 */
function encrypt(key, payload, headers) {
  const iv = node_crypto.randomBytes(12);

  const cipher = node_crypto.createCipheriv(ENCRYPTION_ALG, key, iv);
 ...
 */
function decrypt(keystore, jweCompact) {
...
function decryptAttempt(key, protectedHeader, iv, ciphertext, tag) {
  const decipher = node_crypto.createDecipheriv(ENCRYPTION_ALG, key, Buffer.from(iv, 'base64url'));
...
function generateSignature(cookie, value, key) {
  return node_crypto.createHmac('sha256', key)
...
const debug$5 = Debug('cookie-store');
...
class DefaultCookieStore {
  ...
  getCookie(req) {
    ...
    if (sessionName in cookies) {
      ...
      return Object.entries(cookies)
        ...
        .map(([i, chunk]) => {
          debug$5('reading session chunk from %s.%d cookie', sessionName, i);
          return chunk;
        })
        .join('');
    }
  ...
  setCookie(req, res, { uat = epoch(), iat = uat, exp = this.calculateExp(iat, uat) }) {
    ...
    if (!session) {
      debug$5('session was deleted or is empty, clearing all matching session cookies');
    ...
    debug$5('found session, creating signed session cookie(s) with name %o(.i)', sessionName);
    ...
    if (chunkCount > 1) {
      debug$5('cookie size greater than %d, chunking', this.cookieChunkSize);
      ...
      if (sessionName in cookies) {
        debug$5('replacing non chunked cookie with chunked cookies');
        this.clearCookie(sessionName, res);
      }
    } else {
      res.cookie(sessionName, value, options);
      for (const cookieName of Object.keys(cookies)) {
        debug$5('replacing chunked cookies with non chunked cookies');
        if (cookieName.match(`^${sessionName}\\.\\d$`)) {
          this.clearCookie(cookieName, res);
        }
      }
...
class Session {
  ...
  assertExpired(rollingDuration, absoluteDuration) {
    ...
    node_assert.strict(exp > epoch, 'it is expired based on options when it was established');

    if (rollingDuration) {
      node_assert.strict(uat + rollingDuration > epoch, 'it is expired based on current rollingDuration rules');
    }
    if (absoluteDuration) {
      node_assert.strict(iat + absoluteDuration > epoch, 'it is expired based on current absoluteDuration rules');
    }
  }
}
...
function appSession(config) {
  ...
  return async function appSessionHandler(req, res, next) {
    if (sessionName in req) {
      debug$4('request object (req) already has %o property, this is indicative of a middleware setup problem', sessionName);
    ...
    if (session) {
      try {
        ...
        if (err instanceof node_assert.AssertionError) {
          debug$4('existing session was rejected because', err.message);
        } else {
          debug$4('unexpected error handling session', err);
        }
      }
    }
    ...
    onHeaders(res, () => {
      try {
        ...
        debug$4('Error setting cookie in onHeaders:', err);
      }
    });
...
async function createClient(config) {
  debug$3('Creating client for issuer %s', config.issuerBaseURL);

  const discoveredConfiguration = await openidClient.discovery(new URL(config.issuerBaseURL), config.clientID, config.clientSecret, undefined, {
  ...
  debug$3('Discovery successful for %s', config.issuerBaseURL);
...
function validateConfiguration(config, serverMetadata) {
  ...
  if (!issuerRespTypes.includes(configRespType)) {
    debug$3('Response type %o is not supported by the issuer. Supported response types are: %o.', configRespType, issuerRespTypes);
  }
  ...
  if (configRespMode && !issuerRespModes.includes(configRespMode)) {
    debug$3('Response mode %o is not supported by the issuer. Supported response modes are %o.', configRespMode, issuerRespModes);
  }
...
class ResponseContext {
  ...
  silentLogin(options) {
    ...
    debug$2('Attempting silent login');
  ...
  async login(options) {
    ...
    try {
      ...
      if (options?.returnTo) {
        ...
        debug$2('req.oidc.login() called with returnTo: %s', returnTo);
        ...
        debug$2('req.oidc.login() without returnTo, using: %s', returnTo);
      }
      ...
      node_assert.strict(/\bopenid\b/.test(authOptions.authorizationParams.scope), 'scope should contain "openid"');
      ...
      if (usePKCE) {
        debug$2('response_type includes code, the authorization request will use PKCE');
        authVerification.code_verifier = openidClient.randomPKCECodeVerifier();

        authParams.code_challenge_method = 'S256';
        authParams.code_challenge = await openidClient.calculatePKCECodeChallenge(authVerification.code_verifier);
      }
      ...
      debug$2('redirecting to %s', authorizationUrl);
  ...
  async logout(params) {
    ...
    debug$2('req.oidc.logout() with return url: %s', returnUrl);
    ...
    if (!req.oidc.isAuthenticated()) {
      debug$2('end-user already logged out, redirecting to %s', returnUrl);
    ...
    debug$2('logging out of identity provider, redirecting to %s', returnUrl);
  ...
  async backchannelLogout() {
    ...
    if (this.hasBackchannelLogout && this.backchannelLogoutOptions?.isInsecure) {
      ...
      debug$2('Using insecure backchannel logout mode - DO NOT USE IN PRODUCTION');
      ...
      try {
        ...
        debug$2('req.oidc.backchannelLogout() failed with: %s', e.message);
        res.status(400).json({
    ...
    try {
      ...
      try {
        ...
        debug$2('Logout token verified successfully');
      } catch (verificationError) {
        debug$2('Logout token verification failed: %s', verificationError.message);
        res.status(400).json({
          error: 'invalid_token',
      ...
      debug$2('Backchannel logout error: %s', err.message);
      res.status(500).json({
        error: 'server_error',
  ...
  async getLogoutUrl(returnUrl, idTokenHint, logoutParams) {
    ...
    if (!config.idpLogout) {
      debug$2('performing a local only logout, redirecting to %s', returnUrl);
      return returnUrl;
    }
...
exports.DefaultCookieStore = DefaultCookieStore;
